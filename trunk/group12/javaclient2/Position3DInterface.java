 ...
 *
 * $Id: Position3DInterface.java,v 1.7 2006/03/10 19:05:00 veedee Exp $
 *
 */
package javaclient2;
...
public class Position3DInterface extends AbstractPositionDevice {
  ...
  /**
     ...
     * @param pos a PlayerPose3d structure containing the required data
     ...
     * @param state motor state (zero is either off or locked, depending on the driver)
     */
    public void setVelocity (PlayerPose3d pos, int state) {
        try {
        	sendHeader (PLAYER_MSGTYPE_CMD, PLAYER_POSITION3D_CMD_SET_POS, 24+4);
        	XdrBufferEncodingStream xdr = new XdrBufferEncodingStream (24+4);
        	xdr.beginEncoding (null, 0);
    ...
    protected void handleResponse (PlayerMsgHdr header) {
        switch (header.getSubtype ()) {
            ...
            default:{
            	if (isDebugging)
            		System.err.println ("[Position3D][Debug] : " +
